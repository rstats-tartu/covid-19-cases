...
## positiivsete testide osakaal kumulatiivse keskmisena

```{r}
ggplot(est_summary_3 %>% filter(resultsdate > "2020-03-26"), aes(resultsdate, cu_frac, color=County)) + geom_line()+  
  geom_dl(aes(label = County), method = list("last.points", cex = 0.8)) +
  theme(legend.position = "none", axis.title.x = element_blank())
```

## sama liikuva keskmisega silutud
```{r}
